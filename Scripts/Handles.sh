...
#DAE_DIR="$PKG_PATH/../feeds/packages/net/dae"
#if [ -d "$DAE_DIR" ]; then
#   rm -fr  "$DAE_DIR"
#   echo "删除目录: $DAE_DIR  "
#fi
...
TS_FILE="$(find "$PKG_PATH/../feeds/packages" -maxdepth 3 -type f -wholename '*/tailscale/Makefile' -print -quit 2>/dev/null)"
if [ -f "$TS_FILE" ]; then
...
#升级Xray
XRAY_FILE="$(find "$PKG_PATH/../feeds/packages" -maxdepth 3 -type f -wholename "*/xray-core/Makefile" -print -quit 2>/dev/null)"
if [ -f "$XRAY_FILE" ]; then
	echo " "
